...
const CoinChart: React.FC<TProps> = ({
  prices,
  isFetching,
  changeHistoryMode,
  data,
  ...
  const domainRange = {
    min: priceRange.min
      ? toRoundValue(priceRange.min - priceRange.min / 10)
      : 0,
    max: priceRange.max
      ? toRoundValue(priceRange.max + priceRange.max / 10)
      : 0,
  }
                      ...
                      <li key={key} className='nav-item'>
                        <button
                          className={classNames('btn', 'nav-link', {
                            active: key === historyMode,
                          })}
                          onClick={() =>
                            changeHistoryMode(key as TCoinHistoryMode)
                          }
                        >
                          {value}
                        </button>
